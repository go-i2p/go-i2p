...
func (r *Router) logInboundI2NPIngress(msg i2np.Message, peerHash common.Hash, session i2npReader) {
	...
	// Gate the expensive estimateI2NPMessageSize call (a full msg.MarshalBinary()
	// serialization) behind an explicit Debug-level check so a high-throughput
	// router doesn't pay a full serialization cost per inbound message when
	// Debug logging is disabled (the common/production case).
...
func (r *Router) routeDatabaseLookup(msg i2np.Message, mr *i2np.I2NPMessageDispatcher, fs *netdb.FloodfillServer) error {
	if fs != nil && fs.IsEnabled() {
		...
		// When floodfill handling is available, it is the authoritative
		// DatabaseLookup path and must not be followed by generic routing,
		// otherwise duplicate/conflicting replies may be emitted.
		if err := fs.HandleDatabaseLookup(lookup); err != nil {
			return oops.Wrapf(err, "floodfill lookup handling failed")
		}
		return nil
	}

...
The bootstrap package performs comprehensive validation on all RouterInfo entries obtained from reseed servers, local files, or netDb directories. This validation ensures that only well-formed, usable peer information enters the router's network database.
...
Validation functions return detailed error messages describing why a RouterInfo or RouterAddress failed validation:

...
func (kr *KademliaResolver) iterativeLookupWithOptions(ctx context.Context, target common.Hash, resolveSuggestedPeers bool, exploration bool) (*router_info.RouterInfo, error) {
	...
	return kr.executeLookupRounds(ctx, target, state, resolveSuggestedPeers, exploration)
}
...
func (kr *KademliaResolver) executeLookupRounds(ctx context.Context, target common.Hash, state *LookupState, resolveSuggestedPeers bool, exploration bool) (*router_info.RouterInfo, error) {
	for hop := 0; hop < MaxIterativeLookupHops; hop++ {
		...
		ri, exhausted := kr.processLookupRound(ctx, target, state, hop, resolveSuggestedPeers, exploration)
		if ri != nil {
			return ri, nil
		}
		if exhausted {
...
func (kr *KademliaResolver) processLookupRound(ctx context.Context, target common.Hash, state *LookupState, hop int, resolveSuggestedPeers bool, exploration bool) (ri *router_info.RouterInfo, exhausted bool) {
	...
	results := kr.queryBatchParallel(ctx, batch, target, resolveSuggestedPeers, exploration)
...
func (kr *KademliaResolver) queryBatchParallel(ctx context.Context, peers []common.Hash, target common.Hash, resolveSuggestedPeers bool, exploration bool) []iterativeQueryResult {
	...
	for _, peer := range peers {
		...
		go func(p common.Hash) {
			...
			ri, err := kr.queryPeer(queryCtx, p, target, resolveSuggestedPeers, exploration)
...
func (kr *KademliaResolver) queryPeer(ctx context.Context, peer, target common.Hash, resolveSuggestedPeers bool, exploration bool) (*router_info.RouterInfo, error) {
	...
	// Note: exploration is now passed as a parameter, not read from kr.exploration.
	kr.mu.RLock()
...
func (kr *KademliaResolver) resolvePeerRouterInfo(ctx context.Context, peerHash common.Hash, allowNetworkResolution bool) (*router_info.RouterInfo, error) {
	...
	// Pass exploration=false to ensure nested peer resolution issues normal lookups
	// (not exploration-type), so we receive DatabaseStore responses for the target peer
	// instead of just DatabaseSearchReply suggestions. This allows the explorer to store
	// the resolved peer directly rather than delegating all growth to the prefetcher.
	peerRI, err := kr.iterativeLookupWithOptions(resolveCtx, peerHash, false, false)
	if err != nil {

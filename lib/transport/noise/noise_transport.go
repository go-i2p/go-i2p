...
// NoiseTransport represents a noise protocol transport for I2P router connections
// Moved from: transport.go
...
// Compatible checks if the transport is compatible with the given router info
// Moved from: transport.go
...
// getSession gets a session (internal method)
// Moved from: transport.go
...
// localStaticIV returns the local static initialization vector
// Moved from: transport.go
...
// HandshakeKey returns the handshake key for the transport
// Moved from: transport.go
func (h *NoiseTransport) HandshakeKey() *noise.DHKey {
	return nil
}

// Handshake performs a noise protocol handshake with the given router
// Moved from: handshake.go

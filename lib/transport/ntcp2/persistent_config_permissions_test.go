...
// TestPersistentConfig_DirectoryPermissions asserts the NTCP2 persistent
// config directory is created with 0o700 so that other local users cannot
// enumerate key/IV filenames. See AUDIT.md MEDIUM — "Transport working
// directory is created with mode 0o755..." (2026-04-16).
func TestPersistentConfig_DirectoryPermissions(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("POSIX permissions not enforced on Windows")
	}
	dir := filepath.Join(t.TempDir(), "ntcp2-working")

...
func createTwoSessions(t *testing.T) (s1, s2 *Session) {
	...
	require.NoError(t, err, "NewSession(1) error")
	t.Cleanup(s1.Stop)
...
func assertQueueRejectsAfterStop(t *testing.T) {
	t.Helper()
	session, err := NewSession(1, nil, nil)
	require.NoError(t, err, "NewSession error")
	session.Stop()
	err = session.QueueIncomingMessage([]byte("test"))
	assert.Error(t, err, "Expected error when queuing to stopped session")
}
...
func assertReceiveNilAfterStop(t *testing.T) {
	...
	assert.Nil(t, msg, "Expected nil message after stop")
}

func TestDefaultSessionConfig(t *testing.T) {
	config := DefaultSessionConfig()

	assert.Equal(t, 3, config.InboundTunnelLength)
	assert.Equal(t, 3, config.OutboundTunnelLength)
	assert.Equal(t, 5, config.InboundTunnelCount)
	assert.Equal(t, 5, config.OutboundTunnelCount)
...
func TestSessionGetters(t *testing.T) {
	...
	assert.NotNil(t, session.Destination(), "Destination() should not be nil")

	createdAt := session.CreatedAt()
	assert.False(t, createdAt.IsZero(), "CreatedAt() should not be zero")
	assert.True(t, time.Since(createdAt) <= time.Second, "CreatedAt() should be recent")

	assert.Nil(t, session.InboundPool(), "InboundPool() should be nil for new session")
	assert.Nil(t, session.OutboundPool(), "OutboundPool() should be nil for new session")

	pool := &tunnel.Pool{}
	session.SetOutboundPool(pool)
	assert.Equal(t, pool, session.OutboundPool(), "OutboundPool() should return the set pool")
}
...
func TestSessionQueueFull(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		require.NoError(t, session.QueueIncomingMessage([]byte("msg")), "QueueIncomingMessage() error at %d", i)
	}

	// Next message should fail
	assert.Error(t, session.QueueIncomingMessage([]byte("overflow")), "Expected error when queue is full")
}
...
func TestSessionStop(t *testing.T) {
	...
	assert.False(t, session.IsActive(), "Session should not be active after Stop()")
	assert.Error(t, session.QueueIncomingMessage([]byte("msg")), "Expected error queuing to stopped session")

	msg, _ := session.ReceiveMessage()
	assert.Nil(t, msg, "ReceiveMessage() from stopped session should be nil")
}
...
func TestSessionReconfigure(t *testing.T) {
	...
	require.NoError(t, session.Reconfigure(newConfig), "Reconfigure() error")

	assert.Equal(t, 10, session.Config().InboundTunnelCount)
	assert.Equal(t, "updated", session.Config().Nickname)
}
...
func TestSessionManager(t *testing.T) {
	...
	assert.True(t, ok, "GetSession() returned false")
	...
	assert.NotEqual(t, session1.ID(), session2.ID(), "Sessions should have different IDs")
	assert.Equal(t, 2, manager.SessionCount())

	// Destroy first session
	require.NoError(t, manager.DestroySession(session1.ID()), "DestroySession() error")
	assert.Equal(t, 1, manager.SessionCount())

	_, ok = manager.GetSession(session1.ID())
	assert.False(t, ok, "Destroyed session should not be retrievable")

	manager.StopAll()
	assert.Equal(t, 0, manager.SessionCount(), "SessionCount() after StopAll()")
}
...
func TestSessionManagerDestroy_NotFound(t *testing.T) {
	...
	assert.Error(t, err, "Expected error destroying non-existent session")
}
...
func TestSessionManager_MultipleCreatesAndDestroys(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		require.NoError(t, err, "CreateSession() iteration %d error", i)

		require.NoError(t, manager.DestroySession(session.ID()), "DestroySession() iteration %d error", i)
	}

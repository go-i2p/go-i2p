...
Advice for Developers: The API is still unstable below `go-i2p/onramp`. In fact, we're going to aggressively remove broken, crappy versions of functions from the low-level components over the `0.1.6` to ``0.2.0` development cycle. The `0.2.0` release will have only the type-safe, memory-safe, functionally-correct versions of everything. We'll apply `DEPRECATED` notices to the godoc of anything we're removing in advance of removing it but it *is going to go*. How much low-level stuff you want to use is going to affect how much your life is going to change in the next ~4 releases. Somewhat tacitly, this means that because `go-i2p/onramp` fully abstracts away the embedded router setup `go-i2p/onramp` will be fully stabilized after the `0.1.5` router release. If we can, we'll backport the embedded router feature from `go-i2p/onramp` to `go-i2p/go-sam-go` so everybody gets embedded routers automatically but for now it's more important that `go-i2p/go-sam-go` stays focused on being a complete, high-quality implementation of the SAMv3.3 API.
